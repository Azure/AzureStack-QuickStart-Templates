						   ...
						   )
{
  ...
  foreach ($key in $ParamSet.Keys)
  {
    ...
    $temp = Start-Job $deploymentBlock -Name $jobName -ArgumentList $TENANT_ID, $sp_creds, $SubscriptionID, $resourceGroupName, $ResourceGroupLocation, $TemplateURI, $ParamSet.Item($key), $RunValidation;
							...
							)
{
		...
		$webpage = Invoke-WebRequest $deployment.Outputs['admin_site'].Value
		...
		$webpage = Invoke-WebRequest $deployment.Outputs['ethereum_rpc_endpoint'].Value
