...
if([string]::IsNullOrEmpty($global:SP_PASSWD)) {
  ...
  Try {
    ...
    if ((Select-AzureRmSubscription -SubscriptionName "Blockchain - Customer Facing").Account.AccountType -ne "User")
    {
    ...
    $global:SP_PASSWD =  (Get-AzureKeyVaultSecret -VaultName RunnerSecrets -Name blockchain-service-principle-dev).SecretValueText;
