...
function VMBootAll {
	...
	# Log file
	$logFile = "$env:SystemDrive\VMBootAll.log.ps1"
...
"@
    ...
    if($azureCreds -eq $null) {
		Write-Host "Powershell Credential object is null. Cannot proceed.";
		echo "Powershell Credential object is null. Cannot proceed." >> $logFile;
		return;
	}
	...
	if($noOfDeployedVMs -lt $vmCount) {
		...
		echo "Requested number $vmCount of VMs are not deployed. Currently $noOfDeployedVMs VMs are deployed." >> $logFile;
	}
	...
	foreach($vm in $vms) {
		...
		if($vm.Name -match "[0-9]$") {
			...
			echo Turning off VM $vm.Name in parallel at $_date >> $logFile;
			#Stop-AzureRmVM -Name $vm.Name -ResourceGroupName $vm.ResourceGroupName -Force
			Start-Job -ScriptBlock {
				param($_vmName,$_resourceGroupName,$applicationId,$tenant,$azureUserName,$azurePassword)
...
"@
				...
				$_subdetails = $_resourceGroupName;
				Write-Host Subscription details: $_subdetails -ForegroundColor Yellow;

			} -ArgumentList $vm.Name,$vm.ResourceGroupName,$applicationId,$tenant,$azureUserName,$azurePassword
	...
	if($noOfRunningVMs -gt 0) {
		...
		echo "$noOfRunningVMs VMs failed to turn off out of $vmCount VMs." >> $logFile;
	}
	else {
		Write-Host "All VMs are turned off";
		echo "All VMs are turned off" >> $logFile;
	}
	...
	foreach($vm in $vms) {
		...
		if($vm.Name -match "[0-9]$") {
			...
			echo Booting VM $vm.Name at $_date >> $logFile;
...
"@
				...
				# Azure returns null value if script invoked as a scheduled task
				$_ret = Start-AzureRmVM -Name $_vmName -ResourceGroupName $_resourceGroupName;

				# Get VM Boot End Time (Ignore NULL Time values)
	...
	if($vmbootResults.Count -gt 0) {
		...
		echo "Failed to get VM boot results" >> $logFile;
	...
	Publish-AzureRmVMDscConfiguration -ResourceGroupName $resourceGroupName -ConfigurationPath $logFile -StorageAccountName $storageAccount -SkipDependencyDetection -Force;
}
VMBootAll