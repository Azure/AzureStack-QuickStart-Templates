# Create Remote Desktop Sesson Collection deployment using existing AD and custom os image for rdsh role.
...
+	One (or more) VMs configured as RDSH host role using custom image. NOTE: Because HA is not supported on AzureStack TP2, please use only one VM or it will fail.
